      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-panel dark:glass-panel-dark p-6 rounded-[2rem] premium-shadow border-none">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-extrabold text-[#0a0f5c] dark:text-white">Semester Progress</h2>
            <span className="text-sm font-bold text-[#2dd4a8]">Week 8 of 13</span>
          </div>
        ...
        <div className="glass-panel dark:glass-panel-dark p-6 rounded-[2rem] premium-shadow border-none flex items-center justify-center bg-gradient-to-br from-[#0a0f5c] to-[#1a2080]">
           <div className="text-center">
             <p className="text-xs font-bold uppercase tracking-widest text-[#2dd4a8]/80">Current GPA</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-extrabold text-[#0a0f5c] dark:text-white">Next Up</h2>
            <div className="flex items-center gap-2">
              ...
              <button className="text-xs font-bold text-[#0a0f5c] dark:text-[#2dd4a8] hover:underline">VIEW FULL TIMETABLE</button>
            </div>
          </div>
            ...
            >
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h3 className="text-base font-bold text-[#0a0f5c] dark:text-white leading-tight">{lecture.subject}</h3>
                  <div className="mt-3 flex items-center gap-4 text-xs text-slate-600 dark:text-slate-400 font-medium">
                    <span className="flex items-center gap-1.5">
                      <Clock size={14} className="text-[#0a0f5c] dark:text-[#2dd4a8]" />
                      {lecture.time}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <MapPin size={14} className="text-[#0a0f5c] dark:text-[#2dd4a8]" />
                      {lecture.location}
                    </span>
                  </div>
                </div>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-extrabold text-[#0a0f5c] dark:text-white mb-2">Notice Board</h2>
          
          <div className="glass-panel dark:glass-panel-dark p-5 rounded-2xl premium-shadow border-none space-y-4">
             {[
               { icon: AlertCircle, color: "rose", text: "Lecture moved: Hall A → Hall B4", label: "Academic" },
               { icon: BookOpen, color: "amber", text: "Assignment 3 Slides uploaded", label: "Coursework" },
               { icon: CheckCircle2, color: "emerald", text: "Attendance registered for CS301", label: "System" },
             ].map((note, i) => (
               <div key={i} className="flex items-start gap-4">
                 <div className={`p-2 rounded-xl bg-${note.color}-500/10 text-${note.color}-500 flex-shrink-0`}>
                   <note.icon size={20} />
                 </div>
                 <div className="flex-1 min-w-0">
                    <p className="text-xs font-black uppercase text-slate-400 dark:text-slate-500 mb-0.5">{note.label}</p>
                    <p className="text-sm font-semibold text-[#0a0f5c] dark:text-slate-200 leading-snug">{note.text}</p>
                 </div>
               </div>
             ))}
          ...
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
             <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-slate-600 dark:text-slate-400">CREDIT HOURS</span>
                <span className="text-xs font-black text-[#0a0f5c] dark:text-[#2dd4a8]">102 / 120</span>
             </div>
             <div className="h-1.5 w-full bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden">
                <div className="h-full bg-[#0a0f5c] dark:bg-[#2dd4a8]" style={{ width: "85%" }} />
             </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-extrabold text-[#0a0f5c] dark:text-white mb-2">Deadlines</h2>
          
          {[
            { title: "Database Schema Design", date: "TOMORROW, 11:59PM", urgency: "high" },
            { title: "Network Protocols Quiz", date: "FEB 24, 10:00AM", urgency: "med" },
            { title: "Soft Skills Reflection", date: "MAR 02, 09:00AM", urgency: "low" }
          ].map((item, i) => (
            <div key={i} className="group p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-[#0a0f5c] dark:hover:border-[#2dd4a8] transition-all cursor-pointer">
              <div className="flex justify-between items-start mb-2">
                <div className={`p-1.5 rounded-lg ${
                  item.urgency === 'high' ? 'bg-rose-50 text-rose-500 dark:bg-rose-950/30' : 
                  item.urgency === 'med' ? 'bg-amber-50 text-amber-500 dark:bg-amber-950/30' :
                  'bg-emerald-50 text-emerald-500 dark:bg-emerald-950/30'
                }`}>
                  <Clock size={14} />
                </div>
                <ArrowUpRight size={14} className="text-slate-300 group-hover:text-[#0a0f5c] dark:group-hover:text-[#2dd4a8] transition-colors" />
              </div>
              <h4 className="text-sm font-bold text-[#0a0f5c] dark:text-slate-200 group-hover:text-[#0a0f5c] dark:group-hover:text-[#2dd4a8] transition-colors">{item.title}</h4>
              <p className="mt-1 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase">{item.date}</p>
            </div>
          ))}
          ...
          <div className="mt-8 p-6 rounded-[2rem] bg-gradient-to-br from-[#1a2080] to-[#0a0f5c] text-white overflow-hidden relative">
             ...
             <p className="text-[10px] font-black uppercase tracking-widest text-[#2dd4a8]">Exam Season</p>
